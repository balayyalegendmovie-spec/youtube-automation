...
class YouTubeAutomationPipeline:
    ...
    def run(self, language):
        ...
        if not channel_config.get('enabled', True):
            self.logger.info(f"⏭️ {language.upper()} channel disabled. Skipping.")
            return None
        ...
        # Variables to track for Drive backup
        ...
        try:
            ...
            try:
                long_url = uploader.upload(
                    ...
                    title=long_metadata.get('title', topic_data.get('topic', '')),
                ...
                results['errors'].append(f"Long-form upload: {str(e)[:100]}")
            ...
            for j, (short_meta, short_info) in enumerate(zip(shorts_metadata, shorts)):
                ...
                try:
                    ...
                    results['errors'].append(f"Short {j+1}: {str(e)[:100]}")
            ...
            # ════════════════════════════════════════
            github_group_start("☁️ STEP 11: Backing Up to Google Drive")
            step_start = time.time()
            ...
            try:
                ...
                for j in range(len(shorts)):
                    tp = f"{output_dir}/thumbnail_short_{j}.jpg"
                    if os.path.exists(tp):
                        short_thumb_paths.append(tp)
                ...
                drive_result = drive.upload_pipeline_output(
                    ...
                    short_thumbnails=short_thumb_paths,
                    script_text=reviewed_script,
                    output_dir=output_dir
                )

                if drive_result:
                    results['drive_backup'] = drive_result
                    self.logger.info(f"   ☁️ Drive link: {drive_result.get('folder_link', 'N/A')}")

            except Exception as e:
                self.logger.warning(f"   ⚠️ Drive backup failed: {e}")
                results['errors'].append(f"Drive backup: {str(e)[:100]}")

            step_time = time.time() - step_start
            results['steps']['drive_backup'] = {
                'status': 'success' if results.get('drive_backup') else 'skipped',
                'time_seconds': round(step_time, 1)
            }
            github_group_end()
            ...
            # Still try Drive backup even if pipeline failed
            try:
                from src.drive_uploader import DriveUploader
                drive = DriveUploader()
                if drive.enabled and long_video_path and os.path.exists(long_video_path):
                    self.logger.info("☁️ Attempting Drive backup of partial output...")
                    short_thumb_paths = []
                    for j in range(len(shorts)):
                        tp = f"{output_dir}/thumbnail_short_{j}.jpg"
                        if os.path.exists(tp):
                    ...
                    drive.upload_pipeline_output(
                        run_id=run_id,
                        language=language,
                        topic=topic_data.get('topic', 'failed_run'),
                        long_video_path=long_video_path if os.path.exists(str(long_video_path)) else None,
                        shorts=shorts,
                        thumbnail_long_path=long_thumb_path if long_thumb_path and os.path.exists(str(long_thumb_path)) else None,
                        short_thumbnails=short_thumb_paths,
                        script_text=reviewed_script,
                        output_dir=output_dir
                    )
    ...
    def _print_summary(self, results, language):
        ...
        drive_status = results.get('drive_backup', {})
        if drive_status:
            self.logger.info(f"   ☁️ Drive: {drive_status.get('total_size_mb', 0)} MB backed up")
            self.logger.info(f"   🔗 Drive link: {drive_status.get('folder_link', 'N/A')}")
        else:
            self.logger.info(f"   ☁️ Drive: skipped")
    ...
    def _write_github_summary(self, results, language):
        ...
        drive_info = results.get('drive_backup', {})
...
| Drive Backup | {drive_info.get('total_size_mb', 'N/A')} MB |
| Total Time | {results.get('total_time_seconds', 0):.0f}s |
| Status | {'✅ Success' if results.get('status') == 'success' else '❌ Failed'} |
"""
        if drive_info.get('folder_link'):
            summary += f"\n🔗 [Google Drive Backup]({drive_info['folder_link']})\n"
